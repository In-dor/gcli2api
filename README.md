...
- **如需在云服务器或其他远程环境部署，请先在本地运行服务并完成 OAuth 验证，获得生成的 json 凭证文件（位于 `./geminicli/creds` 目录）后，再在 auth 面板将该文件上传即可。**
...
- `RETURN_THOUGHTS_TO_FRONTEND`: 是否将思维链返回到前端（默认：true）
- `REQUEST_THOUGHTS_FROM_MODEL`: 是否强制向模型请求思维链（默认：false）

**⚠️ 注意：**
大部分配置项（如密码、代理、模型配置等）如果在环境变量中设置，Web 控制台中的对应配置将被锁定且无法修改。这是为了防止意外覆盖关键的部署配置。

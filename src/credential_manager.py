...
class CredentialManager:
    ...
    async def initialize(self):
        ...
        async with self._state_lock:
            ...
            log.debug(f"凭证管理器已使用 {storage_type} 存储后端初始化")
    ...
    async def _discover_credentials(self):
        ...
        try:
            ...
            if all_credentials:
                try:
                    ...
                    for credential_name in all_credentials:
                        ...
                        # 标准化文件名以匹配状态数据中的键
                        if hasattr(
                            self._storage_adapter._backend, "_normalize_filename"
                        ):
                            normalized_name = (
                                self._storage_adapter._backend._normalize_filename(
                                    credential_name
                    ...
                    log.warning(f"批量加载凭证状态失败，回退到单独检查: {e}")
                    # 如果批量读取失败，回退到逐个检查
                    for credential_name in all_credentials:
                        try:
                            ...
                            log.warning(f"检查凭证 {credential_name} 的状态失败: {e2}")
            ...
            if old_credentials != new_credentials:
                ...
                if not is_initial_load:
                    ...
                    if available_credentials:
                        log.debug(
                            f"初始加载发现 {len(available_credentials)} 个可用凭证"
                        )
            ...
            if not self._credential_files:
                log.warning("未找到可用的凭证文件")
            else:
                log.debug(f"可用凭证: {len(self._credential_files)} 个文件")

        except Exception as e:
            log.error(f"发现凭证失败: {e}")
    ...
    async def _load_current_credential(self) -> Optional[Tuple[str, Dict[str, Any]]]:
        ...
        try:
            ...
            if not credential_data:
                log.error(f"加载凭证数据失败: {current_file}")
                return None

            # 检查refresh_token
            if (
                "refresh_token" not in credential_data
                or not credential_data["refresh_token"]
            ):
                log.warning(f"凭证 {current_file} 中没有刷新令牌")
                return None
            ...
            if "type" not in credential_data and all(
                ...
                log.debug(f"已为文件 {current_file} 中的凭证自动添加 'type' 字段")
            ...
            for attempt in range(max_attempts):
                try:
                    ...
                    if current_file:
                        ...
                        self._current_credential_index = 0
                        log.info(
                            f"切换到下一个可用凭证 (索引: {self._current_credential_index})"
                        )
                    else:
                    ...
                    log.error(
                        f"获取凭证时发生异常 (尝试 {attempt + 1}/{max_attempts}): {e}"
                    )
    ...
    async def _rotate_credential(self):
        ...
        log.info(f"已轮换到凭证索引 {self._current_credential_index}")
    ...
    async def update_credential_state(
        self, credential_name: str, state_updates: Dict[str, Any]
    ):
        """更新凭证状态"""
        try:
            # 直接通过存储适配器更新状态
            ...
            if success:
                log.debug(f"已更新凭证状态: {credential_name}")
            else:
                log.warning(f"更新凭证状态失败: {credential_name}")

            return success

        except Exception as e:
            log.error(f"更新凭证状态 {credential_name} 时出错: {e}")
            return False
    ...
    async def set_cred_disabled(self, credential_name: str, disabled: bool):
        ...
        try:
            ...
            log.error(f"设置凭证禁用状态 {credential_name} 时出错: {e}")
            return False
        ...
        try:
            ...
            if email:
                ...
                await self.update_credential_state(
                    credential_name, {"user_email": email}
                )
                return email
            ...
            log.error(f"获取用户邮箱 {credential_name} 时出错: {e}")
            return None
    ...
    async def record_api_call_result(
        ...
        try:
            ...
            log.error(f"记录API调用结果 {credential_name} 时出错: {e}")
    ...
    # 兼容性方法 - 保持与现有代码的接口兼容
    async def _update_token_in_file(
        self, file_path: str, new_token: str, expires_at=None
    ):
        """更新凭证令牌（兼容性方法）"""
        try:
            credential_data = await self._storage_adapter.get_credential(file_path)
            if not credential_data:
                log.error(f"未找到用于令牌更新的凭证: {file_path}")
                return False
            ...
            success = await self._storage_adapter.store_credential(
                file_path, credential_data
            )

            if success:
                log.debug(f"凭证的令牌已更新: {file_path}")
            else:
                log.error(f"未能更新凭证的令牌: {file_path}")

            return success

        except Exception as e:
            log.error(f"更新令牌 {file_path} 时出错: {e}")
            return False

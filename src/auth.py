...
async def get_callback_port():
    """获取OAuth回调端口"""
    return int(
        await get_config_value("oauth_callback_port", "8080", "OAUTH_CALLBACK_PORT")
    )


# 全局状态管理 - 严格限制大小
auth_flows = {}  # 存储进行中的认证流程
auth_flows_lock = threading.Lock()  # 保护auth_flows的线程安全锁
...
def cleanup_auth_flows_for_memory():
    """清理认证流程以释放内存"""
    global auth_flows
    with auth_flows_lock:
        cleaned = cleanup_expired_flows(lock_acquired=True)
        # 如果还是太多，强制清理一些旧的流程
...
class AuthCallbackHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        # 使用锁保护对auth_flows的访问
        with auth_flows_lock:
            is_valid_state = state in auth_flows
            if is_valid_state:
                # 更新流程状态
                auth_flows[state]["code"] = code
                auth_flows[state]["completed"] = True

        if code and state and is_valid_state:
    ...
    try:
        ...
        with auth_flows_lock:
            ...
            auth_flows[state] = {
                ...
                "auto_project_detection": project_id
                is None,  # 标记是否需要自动检测项目ID
                "get_all_projects": get_all_projects,  # 是否为所有项目获取凭证
            }

            # 清理过期的流程（30分钟）
            cleanup_expired_flows(lock_acquired=True)
    ...
    while time.time() - start_time < timeout:
        ...
        with auth_flows_lock:
            if state in auth_flows:
                ...
                # 流程可能已被清理
                log.warning(f"等待过程中认证流程 {state} 已消失")
                return None
            ...
            if flow_data.get("get_all_projects", False) or get_all_projects:
                ...
                if user_projects:
                    ...
                    async def process_single_project(project_info):
                        ...
                        project_name = (
                            project_info.get("displayName") or project_id_current
                        )
                    ...
                    tasks = [
                        process_single_project(project_info)
...
def cleanup_expired_flows(lock_acquired=False):
    """清理过期的认证流程"""

...
# 本地模块 - 工具和认证
from src.utils import get_base_model_from_feature_model, authenticate_flexible
...
async def get_antigravity_models_with_features(show_variants: bool = True):
    """
    获取 Antigravity 模型列表并添加功能前缀

    Args:
        show_variants: 是否显示变体模型（假流式/流式抗截断），默认为True
    ...
    log.info(
        f"[ANTIGRAVITY MODEL LIST] 生成了 {len(models)} 个模型（包含功能前缀: {show_variants}）"
    )
    return models
...
async def list_gemini_models(token: str = Depends(authenticate_flexible)):
    ...
    models = await get_antigravity_models_with_features(show_variants=show_variants)
    log.info(f"[ANTIGRAVITY MODEL LIST] 返回 Gemini 格式 (show_variants={show_variants})")
    return JSONResponse(
...
async def list_openai_models(token: str = Depends(authenticate_flexible)):
    ...
    models = await get_antigravity_models_with_features(show_variants=show_variants)
    log.info(f"[ANTIGRAVITY MODEL LIST] 返回 OpenAI 格式 (show_variants={show_variants})")

...
class FileStorageManager:
    ...
    async def _migrate_json_to_toml(self) -> None:
        ...
        try:
            ...
            if not json_files and not has_old_state:
                log.debug(
                    "No JSON credential files or old state file found for migration"
                )
                return
            ...
            if os.path.exists(self._state_file):
                try:
                    async with aiofiles.open(
                        self._state_file, "r", encoding="utf-8"
                    ) as f:
                        content = await f.read()
                    if content.strip():
            ...
            if has_old_state:
                try:
                    async with aiofiles.open(
                        old_state_file, "r", encoding="utf-8"
                    ) as f:
                        content = await f.read()
            ...
            for filename in json_files:
                try:
                    ...
                    if filename in old_state_data and isinstance(
                        old_state_data[filename], dict
                    ):
            ...
            if migrated_count > 0:
                try:
                    ...
                    async with aiofiles.open(
                        self._state_file, "w", encoding="utf-8"
                    ) as f:
                        await f.write(toml_content)
                    ...
                    log.info(
                        f"Migration completed: {migrated_count} files migrated to TOML format"
                    )
    ...
    async def get_credential(self, filename: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # 提取凭证数据（排除状态字段）
            credential_data = {
                k: v for k, v in section_data.items() if k not in self.STATE_FIELDS
            }
    ...
    async def get_credential_state(self, filename: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # 提取状态字段
            state_data = {
                k: v for k, v in section_data.items() if k in self.STATE_FIELDS
            }
    ...
    async def update_usage_stats(
        self, filename: str, stats_updates: Dict[str, Any]
    ) -> bool:
        """更新使用统计"""
        self._ensure_initialized()
    ...
    async def get_all_usage_stats(self) -> Dict[str, Dict[str, Any]]:
        ...
        try:
            ...
            for filename, section_data in all_data.items():
                # 提取统计字段
                stats_data = {
                    k: v for k, v in section_data.items() if k in stats_fields
                }

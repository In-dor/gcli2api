...
async def get_creds_status_common(
    ...
    if hasattr(storage_adapter._backend, "get_credentials_summary"):
        result = await storage_adapter._backend.get_credentials_summary(
            ...
            error_code_filter=(
                error_code_filter if error_code_filter and error_code_filter != "all" else None
            ),
            cooldown_filter=(
                cooldown_filter if cooldown_filter and cooldown_filter != "all" else None
            ),
        )
...
async def antigravity_cred_action(
    request: CredFileActionRequest, token: str = Depends(verify_panel_token)
):
...
async def verify_credential_project_common(
    filename: str, is_antigravity: bool = False
) -> JSONResponse:
    ...
    project_id = await fetch_project_id(
        access_token=credentials.access_token, user_agent=user_agent, api_base_url=api_base_url
    )
    ...
    if project_id:
        ...
        # 检验成功后自动解除禁用状态并清除错误码
        await storage_adapter.update_credential_state(
            filename, {"disabled": False, "error_codes": []}, is_antigravity=is_antigravity
        )

        log.info(
            f"检验{cred_type}凭证成功: {filename} - Project ID: {project_id} - 已解除禁用并清除错误码"
        )
...
async def verify_antigravity_credential_project(

...
async def serve_control_panel(request: Request):
    ...
    try:
        ...
        # log.info(f"Serving control panel to user-agent: {request.headers.get('user-agent', '')}")
...
async def verify_auth_token_endpoint(token: str = Depends(verify_token)):
    """验证当前Token是否有效"""
    return JSONResponse(content={"valid": True, "message": "Token有效"})
